	...
	class Lang {
		...
		public static function load( $file , $range = '' ) {
			...
			foreach ( $file as $_file ) {
				$_lang = is_file( $_file ) ? include $_file : array();
		...
		public static function detect() {
			...
			if ( isset( $_GET[ $langDetectVar ] ) ) {
				...
				\Idesign\Cookie::set( $langCookieVar , $langSet , 3600 );
			} elseif ( \Idesign\Cookie::get( $langCookieVar ) ) {
				// 获取上次用户的选择
				$langSet = strtolower( \Idesign\Cookie::get( $langCookieVar ) );
			...
			if ( in_array( $langSet , \Idesign\Config::get( 'lang_list' ) ) ) {
				// 合法的语言
				self::$range = $langSet;
			}
		}
