			...
			if ( isset( $_POST['jFormer'] ) && $this->id == $_POST['jFormerId'] || isset( $_POST['jFormerTask'] ) ) {
				...
				if ( isset( $_POST['jFormer'] ) ) {
					...
					if ( !$this->validationPassed ) {
						...
						try {
							if ( isset( $this->onSubmitFunctionServerSide[0] ) ) {
								$class = new $this->onSubmitFunctionServerSide[0];
								$method = new \ReflectionMethod( $class , $this->onSubmitFunctionServerSide[1] );
								if ( $method->isPublic() && !$method->isStatic() ) {
									$onSubmitResponse = $method->invokeArgs( $class , array( $this->getData() ) );
								} else {
