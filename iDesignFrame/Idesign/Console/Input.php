<?php
	...
	use Idesign\Console\Input\Definition;
	use Idesign\Console\Input\Argument;
		...
		public function __toString() {
			$tokens = array_map( function ( $token ) {
				if ( preg_match( '{^(-[^=]+=)(.+)}' , $token , $match ) ) {
					return $match[1] . call_user_func( array( __CLASS__ , 'escapeToken' ) , $match[2] );
					//return $match[1] . $this->escapeToken( $match[2] );
				}

				if ( $token && $token[0] !== '-' ) {
					return call_user_func( array( __CLASS__ , 'escapeToken' ) , $token );
