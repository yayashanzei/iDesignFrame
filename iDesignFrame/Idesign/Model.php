	...
	class Model {
		...
		public function __call( $method , $args ) {
			if ( in_array( strtolower( $method ) , array( 'count' , 'sum' , 'min' , 'max' , 'avg' ) , true ) ) {
				...
				throw new \Idesign\MyException( __CLASS__ . ':' . $method . ' method not exist' );
			}
		}
		
		// 回调方法 初始化模型
		protected function _initialize() {
			Cache::connect( array( 'path' => CACHE_DIR . DATA_DIR . DS . '_fields' . DS ) );
		}
		...
		// 插入数据前的回调方法
		protected function _before_insert( &$data , $options = array() ) { }
		
		// 插入成功后的回调方法
		protected function _after_insert( $data , $options = array() ) { }
		...
		// 更新数据前的回调方法
		protected function _before_update( &$data , $options = array() ) { }
		
		// 更新成功后的回调方法
		protected function _after_update( $data , $options = array() ) { }
		 ...
		 * @return array
		 */
		protected function _read_data( $data , $options = array() ) {
			// 检查字段映射
			if ( !empty( $this->map ) ) {
				foreach ( $this->map as $key => $val ) {
					if ( isset( $data[ $val ] ) ) {
		...
		public function data( $data = '' ) {
			...
			if ( is_object( $data ) ) {
				...
				throw new MyException( 'data type invalid' , 10300 );
			}
			$this->data = $data;
			return $this;
		}
