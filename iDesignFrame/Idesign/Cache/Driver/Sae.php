	...
	class Sae {
		...
		public function __construct( $options = array() ) {
			if ( !function_exists( 'memcache_init' ) ) {
				throw new \Idesign\MyException( '请在SAE平台上运行代码。' );
			}
			$this->handler = memcache_init();
			if ( !$this->handler ) {
				throw new \Idesign\MyException( '您未开通Memcache服务，请在SAE管理平台初始化Memcache服务' );
			}
			if ( !empty( $options ) ) {
		...
		private function getKv() {
			...
			if ( !$kv ) {
				$kv = new \SaeKV();
				if ( !$kv->init() ) {
					throw new \Idesign\MyException( '您没有初始化KVDB，请在SAE管理平台初始化KVDB服务' );
				}
			}
			return $kv;
		}
