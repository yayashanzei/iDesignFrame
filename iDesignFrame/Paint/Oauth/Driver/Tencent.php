<?php
	...
	class Tencent extends Driver {
		...
		protected function parseToken( $result ) {
			...
			$data = array_merge( $data , array( 'openid' => $_GET['openid'] , 'openkey' => $_GET['openkey'] ) );
			if ( $data['access_token'] && $data['expires_in'] && $data['openid'] ) {
				return $data;
			} else {
				throw new \Idesign\MyException( "获取腾讯微博 ACCESS_TOKEN 出错：{$result}" );
			}
		...
		public function getOauthInfo() {
			...
			if ( !isset( $data['error_code'] ) ) {
				...
				throw new \Idesign\MyException( "获取人人网用户信息失败：{$data['error_msg']}" );
			}
		}
