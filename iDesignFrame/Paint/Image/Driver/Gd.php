<?php
	...
	class Gd {
		...
		public function open( $imgname ) {
			//检测图像文件
			if ( !is_file( $imgname ) ) {
				throw new MyException( '不存在的图像文件' );
			}
		...
		public function save( $imgname , $type = null , $interlace = true ) {
			if ( empty( $this->im ) ) {
				throw new MyException( '没有可以被保存的图像资源' );
			}
		...
		public function width() {
			if ( empty( $this->im ) ) {
				throw new MyException( '没有指定图像资源' );
			}
		...
		public function crop( $w , $h , $x = 0 , $y = 0 , $width = null , $height = null ) {
			if ( empty( $this->im ) ) {
				throw new MyException( '没有可以被裁剪的图像资源' );
			}
		...
		public function thumb( $width , $height , $type = THINKIMAGE_THUMB_SCALING ) {
			if ( empty( $this->im ) ) {
				throw new MyException( '没有可以被缩略的图像资源' );
			}
			...
			switch ( $type ) {
				...
				default:
					throw new MyException( '不支持的缩略图裁剪类型' );
			}
		...
		public function water( $source , $locate = THINKIMAGE_WATER_SOUTHEAST ) {
			//资源检测
			if ( empty( $this->im ) ) {
				throw new MyException( '没有可以被添加水印的图像资源' );
			}

			if ( !is_file( $source ) ) {
				throw new MyException( '水印图像不存在' );
			}

			//获取水印图像信息
			$info = getimagesize( $source );
			if ( false === $info || ( IMAGETYPE_GIF === $info[2] && empty( $info['bits'] ) ) ) {
				throw new MyException( '非法水印文件' );
			}
			...
			switch ( $locate ) {
				...
				default:
					...
					if ( is_array( $locate ) ) {
						...
						throw new MyException( '不支持的水印位置类型' );
					}
			}
		...
		public function text( $text , $font , $size , $color = '#00000000' ,
			...
			//资源检测
			if ( empty( $this->im ) ) {
				throw new MyException( '没有可以被写入文字的图像资源' );
			}

			if ( !is_file( $font ) ) {
				throw new MyException( "不存在的字体文件：{$font}" );
			}
			...
			switch ( $locate ) {
				...
				default:
					...
					if ( is_array( $locate ) ) {
						...
						throw new MyException( '不支持的文字位置类型' );
					}
			}
